...
model1 <- lm(log10(bpmile) ~ Vert +
               ...
               MaxHR + 
               Poles,foo)
foo$resid <- scale(model1$residuals)
...
```{r}
summary(MASS::rlm(log10(bpgam) ~ DateNew,foo))
...
```{r}
ma <- smooth::sma(model1$resid,ic = 'AICc')
...
g <- g+ ggplot2::geom_line()
g <- g+ ggplot2::scale_x_date(date_breaks='14 weeks')
